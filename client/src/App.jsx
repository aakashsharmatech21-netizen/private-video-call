...
export default function App() {
  ...
  const createPeer = (stream) => {
    ...
    pc.onicecandidate = (e) => {
      if (e.candidate) socketRef.current.emit('ice-candidate', { roomId: inputId, candidate: e.candidate });
    };
    return pc;
  };
  ...
  return (
    <div style={{
      ...
      {status === 'idle' && (
        <div style={{
          ...
          <button onClick={generateRoom} style={{
            ...
            color: '#fff', fontSize: 15, fontWeight: 600, cursor: 'pointer', marginBottom: '1rem'
          ...
          <button onClick={joinRoom} style={{
            ...
            border: '1px solid rgba(99,102,241,0.5)', background: 'transparent',
      ...
      {status === 'full' && (
        <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 12, padding: '1rem 2rem', color: '#fca5a5' }}>
          Room is full — only 2 people allowed per call.
        </div>
      )}

      {status === 'waiting' && (
        <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 12, padding: '1rem 2rem', color: '#fcd34d', marginBottom: '1rem' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{
            ...
            <button onClick={() => navigator.clipboard.writeText(roomId)}
              style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: 13 }}>Copy</button>
          </div>
            ...
            <div style={{
              ...
              {[
                ...
                <button key={label} onClick={onClick} style={{
                  ...
                  color: active ? '#fff' : '#fca5a5', fontSize: 12, fontWeight: 500
                }}>
              ...
              {/* Chat toggle button */}
              <button onClick={chatOpen ? () => setChatOpen(false) : openChat} style={{
                display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
                padding: '12px 20px', borderRadius: 12, border: 'none', cursor: 'pointer',
                background: chatOpen ? 'rgba(99,102,241,0.3)' : 'rgba(255,255,255,0.08)',
                color: '#a5b4fc', fontSize: 12, fontWeight: 500, position: 'relative'
              }}>
        ...
        {chatOpen && (
          <div style={{
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {messages.length === 0 && (
                <p style={{ color: '#475569', fontSize: 13, textAlign: 'center', marginTop: '2rem' }}>No messages yet. Say hi! 👋</p>
              )}
              {messages.map((msg, i) => (
            ...
            <div style={{ padding: '12px', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', gap: 8 }}>
              ...
              <button onClick={sendMessage} style={{
                ...
                background: 'rgba(99,102,241,0.7)', color: '#fff', cursor: 'pointer', fontSize: 16
              }}>➤</button>
            </div>
          </div>
        )}
